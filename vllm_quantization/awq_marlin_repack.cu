...
// <NT> https://github.com/IST-DASLab/marlin/marlin/__init__.py
...
__global__ void awq_marlin_repack_kernel(
  ...
  // <NT> k方向总tile个数 和 n方向总tile个数
  // [tile_k_size, tile_n_size] = [16, 64]
  int k_tiles = size_k / tile_k_size; 
  int n_tiles = size_n / tile_n_size;
  // <NT> gridDim.x为block个数，launch时被设为了sm个数，即一个sm一个block
  // block size是一维256个线程。tile的个数 除以 block个数，即每个block（每个sm）负责 block_k_tiles 个tile，
  int block_k_tiles = div_ceil(k_tiles, gridDim.x); 

  // <NT> 以block为单位计算，start_k_tile为每个block的第一个tile
  int start_k_tile = blockIdx.x * block_k_tiles;
  if (start_k_tile >= k_tiles) {
    return;
  }

  // <NT> 取出每个block负责的最后一个tile
  ...
  // <NT> 除以8，表示int32类型的n方向tile数量，因为b_q_weight_ptr以int32进行索引
  constexpr int tile_n_ints = tile_n_size / pack_factor;  // tile_n_size=64, pack_factor=8

  // <NT> 一个warp在tile上的线程分配，n方向用2个线程，k方向用16个，共32个。
  ...
  auto fetch_to_shared = [&](int pipe, int k_tile_id, int n_tile_id) {
    ...
    // <NT> n_tile_id 是 n 方向做multi-stage时当前stage的偏移量
    // b_q_weight_ptr类型为int32，其索引也需要由int4改成int32的, 需要除以pack_factor
    ...
    if (threadIdx.x < stage_size) {
      ...
      // <NT> 异步拷贝16个字节，即4个int，对应一个warp里线程的工作量
  ...
  auto repack_tile = [&](int pipe, int k_tile_id, int n_tile_id) {
    ...
    // <NT> 1个warp 32个线程, 这里launch的一个block有256，即8*32;
    // 这里只使用4个warp，即一半的线程？
    if (warp_id >= 4) {
      return;
    }
    ...
    // <NT> 0,1,8,9是mma的16x8x16指令对B矩元素访问的偏移量，如t0取(0,1,8,9), t1取(2,3,10,11)
  ...
  #pragma unroll
    ...
    // <NT> 针对后续的faster dequant中的4位混插方式，进一步调整
    if constexpr (num_bits == 4) {
      constexpr int pack_idx[8] = {0, 2, 4, 6, 1, 3, 5, 7}; 

      uint32_t res = 0;
  #pragma unroll
      // <NT> 按调整顺序，将每8个数据融合成一个int32
      for (int i = 0; i < 8; i++) {
  ...
  #pragma unroll
  // <NT> 外层沿着k方向进行，stage沿n方向进行。
  for (int k_tile_id = start_k_tile; k_tile_id < finish_k_tile; k_tile_id++) {
    int n_tile_id = 0;

    // <NT> 首次进入新的k_tile，将该k对应的第一个n_tile的所有stage加载到共享内存
    start_pipes(k_tile_id, n_tile_id);

    // <NT> n_tiles是n方向tile的总数
    while (n_tile_id < n_tiles) {
  #pragma unroll
      for (int pipe = 0; pipe < repack_stages; pipe++) {
        // <NT> 如pip为0，则加载第7段到共享内存，(pip=1, load=8),(pip=2, load=9)..(pip=7, load=14)
      ...
      n_tile_id += repack_stages; // <NT> repack_stages为8，分8段进行，
    }
  }
}
  ...
  // <NT> 一个sm一个block，一个block256个线程，一维布局
...
torch::Tensor awq_marlin_repack(torch::Tensor& b_q_weight, int64_t size_k,
  ...
  int blocks;
  // <NT> 获取到该设备有多少个sm，repack kernel的启动是一个sm一个block。
