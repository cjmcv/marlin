...
// <NT> 一般W4A16的发力点是在batchsize较小的情况，这时的gemm将会处于访存受限状态，w16改为w4，放存量减小4倍，理论上能够达到4倍fp16的效果。
// 要选择哪种精度的kernel，主要就是看batchsize的大小。如果batchsize很小，marlin kernel的w4a16应该是足够了的。
...
// <NT> marlin kernel实现的是一个fp16 A x int4/int8 B = fp16 C的混合精度的gemm.
...
// <NT> mma.sync.aligned.m16n8k16.row.col.f32.f16.f16.f32 
...
// <NT> 如实现 res=(a&b)|c, 则lut=(0xf0 & 0xcc) | 0xaa
...
dequant<half, vllm::kU8.id()>(int q) {
  ...
  // <NT> 假设q为4个int8，如1,2,3,4, 即为 (0000 0100) (0000 0011) (0000 0010) (0000 0001)
  ...
  // <NT> 1027和1025分别减去magic num，即1024，得到2和1。
...
__global__ void Marlin(
  ...
  // <NT> 4bit，pack_factor=8，用于int4和int32的索引下标转换
  constexpr int pack_factor = 32 / w_type.size_bits(); 

  // For larger GEMMs we run multiple batchsize 64 versions in parallel for a
  // better partitioning with less reductions
  // <NT> 重新把并行数算出来，prob_m改回大分块的维度，进行计算
  int parallel = 1;
  if (prob_m > 16 * thread_m_blocks) {
    parallel = prob_m / (16 * thread_m_blocks); // <NT> 这里的并行数，即是m方向切分后，进入该kernel的矩阵的基础上，m方向的大分块的数量
    prob_m = 16 * thread_m_blocks;
  }

  int k_tiles = prob_k / 16 / thread_k_blocks; // <NT> k方向 大分块的数量 即block tile
  int n_tiles = prob_n / 16 / thread_n_blocks; // <NT> n方向 大分块的数量
  //<NT> 围绕B矩阵，每个block需要处理大分块的数量：gridDim.x即是block的数量，所有方向大分块block tile的数量乘积 除以 block数量，向上取整
  ...
  // <NT> 使用 % 和 / 的组合，将一维的blockIdx.x转变为二维索引, %用于行索引，则表示列优先，遍历完一列再下一列
  ...
  // <NT> n_tiles是n方向block tile的数量，slice_col_par是n方向block tile的下标。
  ...
  if (slice_col_par >= n_tiles) {
    ...
    // <NT> 因为iters是向上取整得到的，所以有可能会大于n_tiles，取余得到剩余的部分。
  ...
  // <NT> a矩阵gmem的下标索引：将fp16改为int4索引。A/C/scale都是fp16，需要除以8来转INT4。B/zeropoint是int4，需要用32来转INT4
  // a_gl_stride 为 A矩阵一行总数
  int a_gl_stride = prob_k / 8;
  // stride of an A matrix tile in shared memory，
  // <NT> a矩阵smem的下标索引，16*thread_k_blocks是大块数据量，除以8是将fp16下标转为INT4，
  ...
  // <NT> B sizes/strides，prob_n是B矩阵重排之前的n维度的int4总数量，重排后小块的16x16被拉平了，所以需要多乘一个16
  int b_gl_stride = 16 * prob_n / (pack_factor * 4); // 用(pack_factor * 4)来将int4转为INT4，b_gl_stride为一行INT4的总数
  // <NT> 等同于marlin kernel中的 32 * thread_n_blocks / 4，32=16*16/8, 
  ...
  // <NT> Scale sizes/strides without act_order，8是pack_factor，因为scale是fp16，需要除以8来把fp16转为INT4
  ...
  // <NT> Zero-points sizes/strides，int4转INT4下标，zero point是int4，需要除以8再除以4来转成INT4
  ...
  // <NT> a_sh_wr_trans和a_sh_rd_trans存储的都是共享内存里A矩阵的转换后的偏移量。
  ...
  // <NT> 因为B矩阵的访问并不是固定步长的，所以需要在计算过程中计算。
  ...
  auto fetch_to_registers = [&](int k, int pipe) {
    int4* sh_a_stage = sh_a + a_sh_stage * pipe;
  #pragma unroll
    // <NT> A矩阵smem需要读取整个thread_m_blocks维度的内容，即会重复读取。
    ...
    if constexpr (!has_act_order) {
      ...
      if constexpr (group_blocks != -1) {
        if constexpr (group_blocks >= thread_k_blocks) {
          ...
          // <NT> warp下标，256个线程有8个warp，范围是0-7. 
          int warp_id = threadIdx.x / 32;     
          // <NT> n方向warp数量，thread_n_blocks是16x16的块在n方向的数量，256线程下，会有(thread_n_blocks，thread_k_blocks) = (16,4) / (8,8)
          // 如果是(8, 8)， 则 n_warps=2=8/4；
          // 如果是(16, 4)，则 n_warps=4=16/4；
          int n_warps = thread_n_blocks / 4; N方向每个warp处理4个16的tile。n_warps就是n方向需要多少个warp
          // <NT> warp_row(warp_id/n_warps), 
          // 如(8,8)时，0(0/2),0(1/2),1(2/2),1(3/2),3(7/2). 为[4,2] 布局
          // 如(16,8)时，0(0/4),0(1/4),0(2/4),0(3/4),1(7/4). 为[2,4] 布局
          int warp_row = warp_id / n_warps; 
          // <NT> 块为16x16，乘以16换算为k的实际下标。
  ...
  // <NT> b矩阵一个线程for循环4次，每次读取2份frag，一份frag有4个fp16，
  // 即每个线程负责32个fp16，32个线程共1024=32(warp)x32=4x16x16个fp16
  #pragma unroll
    for (int j = 0; j < 4; j++) {
      FragB frag_b0;
      FragB frag_b1;
  ...
  auto thread_block_reduce = [&]() {
    // <NT> b_sh_stride_threads 是B矩阵大分块n方向一行的INT4总数，threads是block内线程总数, 
    ...
    if (red_off >= 1) {
      // <NT> 线程分组，b_sh_stride_threads个线程为一组。
  ...
  #pragma unroll
      // <NT> 遍历m维度上的小块总数，即一个线程会访问m方向所有子块。则下面逻辑均对应m维度上的1行block。
      for (int m_block = 0; m_block < thread_m_blocks; m_block++) { 
  #pragma unroll
        for (int i = red_off; i > 0; i /= 2) {
          // <NT> 限制活跃线程，随着规约轮次的推进，需要参与归于线程会越来越少。
          if (i <= red_idx && red_idx < 2 * i) {
  #pragma unroll
            // <NT> 一个线程处理4*2*4=32个数据，对应一次 matmul 的数据量
            //（FragC frag_c[thread_m_blocks][4][2]，一次matmul得到4x2个fragC）
  ...
  #pragma unroll
                // <NT> 内层j和k合计 4x2x4个数据，与 matmul 中的4x2x4相对应，一个warp 32个线程负责合计处理32x32个数据，即4份16x16的数据。
                for (int k = 0; k < 4; k++) 
                  reinterpret_cast<FragC*>(frag_c)[4 * 2 * m_block + j][k] +=
                      c_rd[k] + c_wr[k];
              }
              // <NT> 一次4个fp32，首次进入无数据，需要提前填充
              sh[red_sh_wr] =
                  reinterpret_cast<int4*>(&frag_c)[4 * 2 * m_block + j]; 
            }
          }
          // <NT> 到这里block内每个warp内每个线程负责的数据量已完成一轮规约。
          // 在下一轮规约时，跨度减半，涉及到当前轮次的跨warp的结果，需要做一次同步。
          __syncthreads();
        }
        // 
        // <NT> 剩下最后一组，上面最后一次规约的frag_c写到了sh[red_sh_wr]的数据
        if (red_idx == 0) {
  #pragma unroll
          for (int i = 0; i < 4 * 2; i++) {
            float* c_rd =
  ...
  auto global_reduce_fp32 = [&](bool first = false, bool last = false) {
    ...
    // <NT> 一个warp处理n方向4份小tile(m方向是单独遍历的)，一个线程则处理连续的4个数据，即一个大int4。
    // 一个warp有32个线程。
    ...
    if (!first) { // <NT> 如果是第一个slice，则不需要跨slice的规约。
      float* frag_c_ptr = reinterpret_cast<float*>(&frag_c);
  #pragma unroll
      for (int k = 0; k < th_size; k++) {
        sh[threadIdx.x] =
            C_tmp[c_cur_offset + active_threads * k + threadIdx.x]; // <NT> 一次一个大int4即4个fp32。
  ...
  #pragma unroll
    ...
    if (!last) {  // <NT> 如果是最后一个slice，则不会再写入C_tmp，而是会进入到write_result函数中，直接写到C里。
  ...
  while (slice_iters) {
    ...
    // index 0.
    // <NT> 循环展开global fetch和寄存器加载pipeline，以确保所有共享内存访问都是静态的。
  ...
  #pragma unroll
      ...
      // <NT> matmul中fragC不涉及for循环的k，即算到这里fragC已经完成了b_sh_wr_iters的累加规约。
    }
...
// <NT> 优先采用256线程一个block，因为256个线程有8个warp。
...
void marlin_mm(const void* A, const void* B, void* C, void* C_tmp, void* s,
  ...
  int tot_m = prob_m;
  // <NT> 除以16，向上取整，表示为m方向的小tile数量，小tile为16x16.
  int tot_m_blocks = div_ceil(tot_m, 16); 
  // <NT> 无法凑整的部分补0
  int pad = 16 * tot_m_blocks - tot_m;    

  if (sms == -1) {
    // <NT> 获取sm个数，一个sm分配一个block
  ...
  // <NT> 一个block的线程数量，是256或128，对应8个/4个warp
  int num_threads = exec_cfg.tb_cfg.num_threads; 
  thread_k = exec_cfg.tb_cfg.thread_k;
  thread_n = exec_cfg.tb_cfg.thread_n;

  // <NT> 线程布局是<<<sm, num_thread>>>, 跟 thread_k 和 thread_n 无关。
  // thread_k和thread_n表示一个大分块的大小，（下面的for循环，每次处理一块？）
  // 小分块是16x16，则thread_k_blocks和thread_n_blocks表示小分块的个数。
  int thread_k_blocks = thread_k / 16; 
  int thread_n_blocks = thread_n / 16; 
 
  // <NT> block数量设为与sm数量一致，本质上计算时就是一个sm一次只能调度一个block。
  ...
  for (int i = 0; i < tot_m_blocks; i += exec_cfg.max_m_blocks) {
    int thread_m_blocks = tot_m_blocks - i;  // <NT> 剩余的tile数量 = m方向的总tile[16,16]个数 - 当前到第i的tile
    prob_m = tot_m - 16 * i;                 // <NT> 剩余行数 = 总行数 - 16x当前tile数
    int par = 1;                             // <NT> 先令并行度为1
    if (thread_m_blocks > exec_cfg.max_m_blocks) {   // <NT> max_m_blocks 是按资源情况和布局计算过的一次处理最大的分块数量
      // Note that parallel > 1 currently only works for inputs without any
      // padding
      par = (16 * thread_m_blocks - pad) / (16 * exec_cfg.max_m_blocks); // <NT> 看按最大的max_m_blocks，可以并行执行多少份
      if (par > max_par) par = max_par;     // <NT> 限制最大并行数量
      prob_m = (16 * exec_cfg.max_m_blocks) * par; // <NT> 按新的并行数量调整当前轮次需要处理的prob_m行
...
// <NT> gptq_marlin_gemm / marlin_mm / Marlin
// 
// A：fp16的激活值[m,k]
// B：int4量化后的权重[k,n]
// C: fp16的输出[m,n]
...
torch::Tensor gptq_marlin_gemm(torch::Tensor& a, torch::Tensor& b_q_weight,
  ...
  // <NT> 分组量化，scale和zero的行数即是group的数量
